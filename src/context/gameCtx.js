...
function proceedToNextSeason(state) {
  ...
  return {
    ...
    availableWater: state.availableWater + nextSeason.water,
    availableFert: state.availableFert + nextSeason.availableFert,
    plants: state.plants.map(plant => {
      ...
      return {
        ...
        health: predictHealth > 100 ? 100 : predictHealth
      };
    })
  };
}
...
function removePlant(state, action) {
  ...
  return {
    ...state,
    plants: state.plants.filter(plt => plt.id !== action.payload),
    availableFert: state.availableFert + plantToUpdate.allocated_fertilizer,
    availableWater: state.availableWater + plantToUpdate.allocated_water
  };
}

function allocateResources(state, action) {
  let availableWater =
    action.type === "allocate_water"
      ? state.availableWater - 100
      : state.availableWater;
  let availableFert =
    action.type === "allocate_fertilizer"
      ? state.availableFert - 100
      : state.availableFert;

  return {
    ...state,
    plants: updatePlantsForResources(state.plants, action),
    ...{ availableWater },
    ...{ availableFert }
  };
}
...
function updatePlantsForResources(plants, action) {
  // These can be combined so you only loop through items once
  let indexOfPlant = plants.findIndex(item => item.id === action.payload);
  let plantToUpdate = plants.find(item => item.id === action.payload);
  switch (action.type) {
    case "allocate_water":
      return [
        ...plants.slice(0, indexOfPlant),
        Object.assign({}, plantToUpdate, {
          allocated_water: plantToUpdate.allocated_water + 100
        }),
    ...
    case "allocate_fertilizer":
      return [
        ...plants.slice(0, indexOfPlant),
        Object.assign({}, plantToUpdate, {
          allocated_fertilizer: plantToUpdate.allocated_fertilizer + 100
        }),
        ...plants.slice(indexOfPlant + 1)
      ];
    default:
