...
function AddPlantModal(props){
    ...
    return <Modal open={props.isOpen} closeIcon onClose={props.onClose}>
    <Modal.Header>Add a Plant!</Modal.Header>
    <segment style={{display:"flex", flexDirection:'row', justifyContent:"space-around", padding:40}}>
        <Modal.Content textAlign="center">
            <Select loading={loadingPlantData} placeholder='Select a plant' options={plantData} onChange={(e, {value})=> {
        ...
        <Modal.Description style={{color:"black"}}>
            ...
            <List style={{paddingTop:10}}>
                <List.Item icon='tint' header={"Water"} content={`${plant.plant.water}`}/>
                <List.Item icon='recycle' header={"Fertilizer"} content={`${plant.plant.fertilizer}`} />
            </List>
            )}
        </Modal.Description>
    </segment>
